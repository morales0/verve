            ...
            <LabelContainer>
               {
                  measures?.map((measure, i) => (
                     <MeasureLabel key={`mesaure-label-${measure}`}>{measure}</MeasureLabel>
                  ))
               }
            </LabelContainer>
            <AddSetButton setSize={measures.length} onClick={addSet}>+</AddSetButton>
            <SetGrid>
               {
                  sets?.map((set, i) => (
                     <SetRow key={`setRow-${i}`}>
                        ...
                        {
                           i > 0 && <RemoveSetButton onClick={() => removeSet(i)}>x</RemoveSetButton>
                        }
                     </SetRow>
                  ))
               }
