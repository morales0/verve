...
const WorkoutContainer = (props) => {
  ...
  const api = {
    ...
    completeWorkout: () => {
      ...
      Object.entries(workoutData.data["completedExercises"]).forEach(
        ([id, sets]) => {
          set(
            ref(
              db,
  ...
  if (completing) {
    return <div>Finalizing workout...</div>;
  }

  // Check if workout exists
  if (workoutData.status === "loading") {
    return <div>Finding your workout...</div>;
  } else if (workoutData.status === "creating") {
    return <div>Creating your workout...</div>;
  }
...
export { WorkoutContainer, useWorkout };
