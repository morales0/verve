...
const History = (props) => {
   ...
   useEffect(() => {
      if (workoutHistory.data !== null) {
         ...
         // Create json
         Object.values(workoutHistory.data).reverse().forEach(workout => {
   ...
   const toTimeString = (date) => {
      ...
      return `${!militaryTime && hour > 12 ? hour % 12 : hour}:${min < 10 ? '0' + min : min}${militaryTime ? '' : hour > 12 ? ' PM' : ' AM'}`
   }
                           ...
                           <div className="workoutListDay_container">
                              {
                                 workoutsByDay[toDateKey(day)] ? (
                                    Object.values(workoutsByDay[toDateKey(day)])
                                    .map(workout => (
